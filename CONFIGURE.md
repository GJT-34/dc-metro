...
| `bus_display_mode` | Integer | This is an internal setting for UI layout columns. Options are: <BR><BR>1. All upcoming arrivals across every route at the stop are collected, sorted by arrival time, and displayed on a single screen. Each arrival occupies one row, with the route and direction left-justified and the arrival time right-justified. Up to 4 arrivals are shown (fewer if a header is configured).<BR><BR>2. Arrivals are grouped by route. Each route gets two rows: one showing the route and direction, and one listing up to three upcoming arrival times. If multiple routes serve the stop, each route appears on its own screen. A header showing the stop name is optional. |
...
| --- | --- |
| ![fc1](img/bd1.webp) | ![fc2](img/bd2.webp) |
